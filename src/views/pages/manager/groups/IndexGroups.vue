<template>
	...
	<div class="p-2">
		...
		<Paginator entity="groups" :per-page="4" class="p-2" v-if="groups.length" />
	</div>
...
<script>
	...
	export default {
		beforeMount() {
			Course.api().fetchById(this.$route.params.course_id, '?includes=group.fields.options');
			this.getGroups();
		},
		data() {
			return {
		...
		methods: {
			...
			getGroups() {
				return Group.api().fetch(`?include=group_users,fields&filter[course_id]=${this.$route.params.course_id}`);
			},
			storeGroup(event) {
		...
		computed: {
			...
			groups() {
				return Group.query().with(['group_users', 'fields']).where('course_id', parseInt(this.$route.params.course_id)).findIn(this.$store.getters['pagination/data']('groups')?.items);
			},
			fields() {
				return this.course?.groups_table?.split(',');
			}
		},
		watch: {
			'$route' () {

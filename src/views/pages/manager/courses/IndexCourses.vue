...
<script>
	...
	export default {
		created() {
			User.api().get('/api/user?include=roles,company.courses,companies.courses');
		},
		computed: {
			user() {
				return User.query().with(['roles', 'companies.courses', 'company.courses']).find(this.$store.getters['auth/user'].id);
			}
		}
	}
</script>
